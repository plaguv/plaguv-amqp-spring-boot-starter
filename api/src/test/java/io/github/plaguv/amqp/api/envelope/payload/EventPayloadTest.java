package io.github.plaguv.amqp.api.envelope.payload;
...
class EventPayloadTest {
    ...
    @BeforeEach
    void beforeEach() {
        payloadType = ArticleOrderEvent.class;
        payload = new ArticleOrderEvent(1, 1, 1);
    }
    ...
    void constructorAcceptsOnlyEventContent() {
        ...
        Assertions.assertDoesNotThrow(
                () -> EventPayload.empty(ArticleOrderEvent.class));
    ...
    void constructorAcceptsOnlyMatchingEventContent() {
        Assertions.assertThrows(IllegalArgumentException.class,
                () -> new EventPayload(ArticleUrgentOrderEvent.class, payload));
